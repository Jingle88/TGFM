...
def tgfm_no_sampling_inference_shell(tgfm_data, gene_log_prior, var_log_prior, gene_variant_full_ld, n_components, gene_tissue_pip_threshold, window_name):
	...
	z_expected = np.abs(plot_data['z'])
	LR = np.exp(plot_data['logLR'])

	z_threshold = 2.0
	lr_threshold = 1.0
	good_indices = np.where((z_expected <= z_threshold) & (LR <= lr_threshold))[0]
	problematic_indices = np.where((z_expected > z_threshold) & (LR > lr_threshold))[0]
	print(f"Number of variants removed: {len(problematic_indices[0])} in window {window_name}")

	'''
	z_std_diff = plot_data['z_std_diff']
	# Threshold residuals to identify good-fit indices (i.e. remove red dots)
	residual_threshold = 2.7
	...
	susie_null_init = susieR_pkg.susie_rss(z=z_vec_filtered.reshape((len(z_vec_filtered),1)), R=ld_filtered, n=tgfm_data['gwas_sample_size'], L=n_components, prior_weights=prior_probs_filtered.reshape((len(prior_probs_filtered),1)), estimate_residual_variance=False)
	...
	if np.max(tgfm_obj.alpha_pip) > gene_tissue_pip_threshold:
		...
		p_var_only = np.ones(len(z_vec_filtered))
		p_var_only[:len(tgfm_obj.nominal_twas_z)] = 0.0
		p_var_only = p_var_only/np.sum(p_var_only)
		# Run susie with only variants
		susie_variant_only = susieR_pkg.susie_rss(z=z_vec_filtered.reshape((len(z_vec_filtered),1)), R=ld_filtered, n=tgfm_data['gwas_sample_size'], L=n_components, prior_weights=p_var_only.reshape((len(p_var_only),1)), estimate_residual_variance=False)
		...
		susie_variant_init = susieR_pkg.susie_rss(z=z_vec_filtered.reshape((len(z_vec_filtered),1)), R=ld_filtered, n=tgfm_data['gwas_sample_size'], L=n_components, s_init=init_obj2, prior_weights=prior_probs_filtered.reshape((len(prior_probs_filtered),1)), estimate_residual_variance=False)
